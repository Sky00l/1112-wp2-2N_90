...
app.post('/api/blogs', (req, res) => {
    const { id, img, remote_url, category, title, desc } = req.body;
    console.log('title', title);
    const newBlog = { id, img, remote_url, category, title, desc };
...
app.patch('/api/blogs/:id', (req, res) => {
    const { id } = req.params;
    const { img, remote_url, category, title, desc } = req.body;
    blogList_xx = blogList_xx.map((item) => {
        if (item.id == id) {
            return { id, img, remote_url, category, title, desc }
        }
        return item;
    });
    console.log('blogList_xx', JSON.stringify(blogList_xx));
